...
interface SocketContextProps {
    messages: string[];
    totalMessage: any;
    confirmBooking: (holdId: number, checkin: string, checkout: string, bank_id: number, commission: number) => void;
    cancelBooking: (confirmId: number, checkin: string, checkout: string) => void;
    cancelConfirmBooking: (holdId: number, checkin: string, checkout: string, rejectionReason: string) => void;
...
export const SocketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const cancelConfirmBooking = async (holdId: number, checkin: string, checkout: string, rejectionReason: string) => {
        try {
            setIsLoading(true);
            await axiosClient.post(`${API_BASE_URL}/booking/accept`, {
                id: holdId,
    ...
    const confirmTransfer = async (confirmId: number, checkin: string, checkout: string, commission: any) => {
        try {
            setIsLoading(true)
            await axiosClient.post(`${API_BASE_URL}/booking/transfer`, { id: confirmId, checkin, checkout, commission_rate: commission });
            toast.success('Đã xác nhận thành công');
            fetchNotifications()
        } catch (error) {
            toast.error('Đã xảy ra lỗi, vui lòng thử lại.');
    ...
    const cancelBooking = async (confirmId: number, checkin: string, checkout: string) => {
        try {
            setIsLoading(true)
            await axiosClient.post(`${API_BASE_URL}/booking/cancel`, { id: confirmId, checkin, checkout });
    ...
    const contextValue = useMemo(() => ({ customerList, bankList, handleBookingSubmit, confirmBooking, cancelBooking, cancelConfirmBooking, cancelHold, confirmReceiveMoney, confirmTransfer, confirmHold, fetchDataDetailBookingQR, qr, messages, totalMessage }), [messages, bankList, totalMessage, qr]);
