...
export default function ServiceCardList({
  ...
  const handleStep4Submit = async (data: any) => {
    ...
    const payload = {
      ...
      price_weeknd_delete: data?.price_weeknd_delete,
      ...
      price_season_delete:data?.price_season_delete,
    };

    try {
      const response = await axiosClient.post(
        'https://core-api.thehostie.com/residences',
        payload,
        {}
      );
  ...
  const handleSavePolicy = async (files: File[]) => {
    ...
    try {
      const response = await axiosClient.post(
        'https://core-api.thehostie.com/residences/policy',
        formData,
        {}
      );
      fetchPolicy();
            ...
            >
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <CardContent sx={{ flexGrow: 1 }}>
                  ...
                  {data?.status === 1 && (
                    <Chip label="Chưa hoàn thành" color="warning" sx={{ marginTop: 1 }} />
                  )}
                </CardContent>
                <CardActions>
                  <IconButton
